...
- **Known follow-up (not in this change):** list ordering (`MainViewModel.performScan`) still sorts by `file.lastModified()`, so a SAF copy with a zeroed mtime sorts last even though its date now displays correctly. Re-keying the sort on the derived date changes observable ordering and is deferred.
