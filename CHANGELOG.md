...
**Root cause:** the display half was fixed (`FlightLog.dateFormatted` → `resolveFlightDisplayMillis`), but the list order in `viewmodel/MainViewModel.kt` (`performScan`) was still keyed on raw `file.lastModified()`. Display and sort read two different fields, so a zeroed mtime rendered its real date yet ordered as epoch-0.

Minimal, non-breaking fix (no god-ViewModel refactor):

- **New pure helper `model/FlightDate.kt::flightSortKey(name, mtime, now)`** — returns a total, non-null `Long` ordering key: `resolveFlightDisplayMillis(...)` (the *same* instant the row displays) with a fall-back to raw `mtime` when display resolves to `null` (an "Unknown date" row), so ordering stays total and deterministic instead of a null-key crash. Same JVM-testable seam as the rest of `FlightDate.kt`.
- **`viewmodel/MainViewModel.kt` (`performScan`)** — the scan sort changes from `sortedByDescending { it.file.lastModified() }` to `sortedByDescending { flightSortKey(it.name, it.file.lastModified(), now) }` (`now` captured once per scan). One-line key swap; nothing else in the ViewModel touched. List order now matches the displayed dates (newest flight first) regardless of a zeroed mtime; Unknown-date rows sink together at the bottom.
- **Tests:** `model/FlightDateTest` grows by 4 (20 total): sort key of a zero-mtime SAF copy equals its filename date (not zero); key falls back to raw mtime when nothing resolves; a mixed list including a zero-mtime SAF copy sorts newest-flight-first by derived date and NOT by mtime (with a companion assertion proving a naive mtime sort would sink it); Unknown-date rows sort below dated flights. **What ran:** `FlightDate.kt` + the full real `FlightDateTest.kt` were compiled with `kotlinc` and executed under `org.junit.runner.JUnitCore` in a container (kotlinc + JUnit 4.13.2) — **OK (20 tests)**. **What must run in CI:** the single Android-runtime edit (`MainViewModel.performScan`) can't unit-run without the Android SDK; the "CI — unit tests" workflow (`testDebugUnitTest` on the BOS-HQ runner) compiles the ViewModel and runs the JVM suite on the real toolchain.

ADR: extends [`docs/adr/0010-flight-date-from-filename-not-mtime.md`](docs/adr/0010-flight-date-from-filename-not-mtime.md) (the "sort half" the 2026-07-02 entry deferred). No version bump in this PR (CI auto-bumps on merge; a manual bump folds into the squash, HEAD reads `[skip ci]`, and the release never fires). Operator end-to-end check: scan controller logs where at least one DJI record has a zeroed mtime and confirm the list orders newest-flight-first, matching the displayed dates.

### Fixed — 2026-07-02 — flight date showed "1969" (garbage mtime) — derive from filename, guard the fallback (ADR-0010)

**Symptom (operator-reported, recurring):** a flight log rendered a wild date — most visibly **"31 Dec 1969"**. `FlightLog.dateFormatted` rendered `SimpleDateFormat(...).format(Date(file.lastModified()))`. For SAF entries `file` is a per-scan cache copy; DJI controller `DocumentFile`s frequently report `lastModified() == 0`, so `Date(0)` (the Unix epoch) rendered as "1969" in the operator's Pacific timezone. File mtime is also the *write* time, not the flight datetime, so it was the wrong field even when non-zero.

Display-only, non-breaking fix (no scan/upload/delete/sync behaviour change):

- **New pure module `model/FlightDate.kt`** (JVM-testable, no Android runtime — same seam as `storage/TransferIntegrity.kt` / `upload/UploadOutcome.kt`; lives in `model` because its only consumer is `model/FlightLog.kt`, which avoids a model↔storage package cycle):
  - `deriveFlightDateMillisFromName(name)` — extracts the authoritative flight datetime from the filename. Covers the real shapes ingested: DJI FlightRecord `DJIFlightRecord_2026-06-30_[12-34-56].txt` (all DJI GO/Fly generations), Litchi leading-timestamp CSV, and Airdata date-in-name CSV. Impossible dates (month 13 / day 32) rejected by a non-lenient parse; date-only names anchored at local noon for TZ/DST-stable calendar dates.
  - `isPlausibleTimestamp(millis, now)` — true only in `[2010-01-01, now + 48h]`. A `0`/negative/epoch mtime is never plausible, so it can never render as 1969 (or any misleading date).
  - `resolveFlightDisplayMillis(name, mtime, now)` — filename date (if plausible) → else mtime (if plausible) → else `null`.
- **`model/FlightLog.kt`** — `dateFormatted` now resolves via `resolveFlightDisplayMillis(...)` and returns **"Unknown date"** on `null`. Formatting (`dd MMM yyyy  HH:mm`, `Locale.getDefault()`) unchanged. A legacy file with a valid mtime and no date-in-name still shows the exact same date as before.
- **`storage/FlightLogSource.kt`** — no change needed: the `if (originalMtime > 0) cacheFile.setLastModified(...)` guard added in ADR-0009 already satisfies "only stamp a plausible mtime."
- **Tests:** `model/FlightDateTest` (20 assertions) — filename parsing for each pattern (DJI bracketed/underscore time, Litchi, Airdata date-only), impossible-date rejection, epoch-0/negative/below-floor/future guard, filename-wins-over-mtime, zero-mtime-uses-filename (the 1969 case), legacy mtime passthrough, and unknown→null. **Note:** the full Android `./gradlew test` suite could NOT run in the fix environment (no Android SDK / no JDK on host). The pure logic in `FlightDate.kt` WAS compiled and executed against the Kotlin compiler in a container, mirroring the JUnit assertions 1:1 — **20/20 PASS across 4 time zones (UTC, PDT, IST +5:30, Kiritimati +14).** The single Android-dependent edit (`FlightLog.dateFormatted`) is inspection-verified. CI must run the JVM suite on a real toolchain.
- **Known follow-up (not in this change):** list ordering (`MainViewModel.performScan`) still sorts by `file.lastModified()`, so a SAF copy with a zeroed mtime sorts last even though its date now displays correctly. Re-keying the sort on the derived date changes observable ordering and is deferred. **[Resolved 2026-07-03 — see the entry above (`flightSortKey`); the sort is now keyed on the same derived flight instant the row displays.]**

ADR: [`docs/adr/0010-flight-date-from-filename-not-mtime.md`](docs/adr/0010-flight-date-from-filename-not-mtime.md). No version bump in this PR (CI auto-bumps `android/version.properties` on merge — a manual bump folds into the squash, HEAD reads `[skip ci]`, and the release never fires). Operator end-to-end check: scan controller logs and confirm no "1969" date; DJI records show their filename datetime; any log with neither a filename date nor a plausible mtime shows "Unknown date".
